...
BitmapFontInitPath :: proc(renderer: ^sdl.Renderer, path: string, size: i32) -> (^BitmapFont, bool) {
  ...
  bf.initFontRW = initFontRW
  bf.initFontPath = initFontPath
  bf.calculateTextWidth = calculateTextWidth
  bf.renderText = renderText
  bf.getGlyphs = getGlyphs
  bf.getGlyphHeight = getGlyphHeight
  bf.total_font_size = total_font_size
  bf.close = close
